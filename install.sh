...
# KDE Plasma reads ~/.config/plasma-workspace/env/*.sh at session start (see KDE UserBase)
NC_PLASMA_ENV_QML="${PLASMA_USER_HOME}/.config/plasma-workspace/env/nextcloud-carousel-qml.sh"
NC_QML_SO_NAME="libnextcloudimageprovider.so"
...
# Plasmashell does not load ~/.local/lib/qt6/qml by default; install a Plasma session snippet
# so import org.nextcloud.carousel resolves without sudo. Remove snippet when /usr has the module.
configure_plasma_qml_import_for_user() {
    local user_so="${NC_USER_LOCAL}/lib/qt6/qml/org/nextcloud/carousel/${NC_QML_SO_NAME}"
    local system_so="/usr/lib/qt6/qml/org/nextcloud/carousel/${NC_QML_SO_NAME}"
    if [ -f "$system_so" ]; then
        if [ -f "$NC_PLASMA_ENV_QML" ]; then
            run_as_target_plasma_user rm -f "$NC_PLASMA_ENV_QML"
            echo "Removed user Plasma env snippet (system QML module in /usr): $NC_PLASMA_ENV_QML"
        fi
        return 0
    fi
    if [ ! -f "$user_so" ]; then
    ...
    run_as_target_plasma_user mkdir -p "${PLASMA_USER_HOME}/.config/plasma-workspace/env"
    local tmp
    tmp="$(mktemp)"
    cat > "$tmp" <<'ENVEOF'
#!/bin/sh
# Generated by Nextcloud Carousel install.sh — user-local Qt 6 QML import path for plasmashell
export QML_IMPORT_PATH="$HOME/.local/lib/qt6/qml${QML_IMPORT_PATH:+:$QML_IMPORT_PATH}"
ENVEOF
    run_as_target_plasma_user install -m 755 "$tmp" "$NC_PLASMA_ENV_QML"
    rm -f "$tmp"
    echo ""
    echo "═══════════════════════════════════════════════════════════"
    echo "  Plasma session: QML path for user-local C++ (no sudo)"
    echo "═══════════════════════════════════════════════════════════"
    echo "Installed: $NC_PLASMA_ENV_QML"
    echo "Log out and log back in (or reboot) so plasmashell picks up QML_IMPORT_PATH."
    echo "Then Nextcloud Carousel can load the C++ downloader instead of QML-only fallback."
    echo "═══════════════════════════════════════════════════════════"
}
...
if [ "$CPP_COMPONENT_AVAILABLE" = true ]; then
    ...
    echo ""
    if [ -f "/usr/lib/qt6/qml/org/nextcloud/carousel/${NC_QML_SO_NAME}" ]; then
        echo "💡 The C++ plugin is installed in /usr/lib/qt6/qml/ (system installation)"
        echo "   Plasmashell will find the QML module without extra configuration."
    ...
    echo ""
    if [ ! -f "/usr/lib/qt6/qml/org/nextcloud/carousel/${NC_QML_SO_NAME}" ] && [ -f "${NC_USER_LOCAL}/lib/qt6/qml/org/nextcloud/carousel/${NC_QML_SO_NAME}" ]; then
        echo "⚠️  If you still see QML after a full session restart, check that"
        echo "   $NC_PLASMA_ENV_QML exists and is executable (chmod +x)."
    fi
else
...
if [ -f "$NC_PLASMA_ENV_QML" ] && [ ! -f "/usr/lib/qt6/qml/org/nextcloud/carousel/${NC_QML_SO_NAME}" ]; then
    echo ""
    echo "   User-local C++: log out and log in once (or reboot) so Plasma applies"
    echo "   QML_IMPORT_PATH from $NC_PLASMA_ENV_QML — then plasmashell will see the module."
fi

# User-local C++: plasmashell needs QML_IMPORT_PATH via plasma-workspace/env (see configure_plasma_qml_import_for_user)
echo ""
...
if [ "${NC_SYSTEM_PLASMA_WALLPAPER}" = true ]; then
    ...
    echo "  Plasma env snippet: rm -f $NC_PLASMA_ENV_QML"
fi
